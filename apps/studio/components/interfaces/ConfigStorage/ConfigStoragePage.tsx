...
export function ConfigStoragePage() {
  ...
  const requestedBranch =
    selectedBranch === null
      ...
      : selectedBranch === REPOSITORY_DEFAULT
        ? undefined
        : selectedBranch
  const branchesReady = (!shouldLoadBranches || !branchesPending) && !connectionPending
  ...
  const { data, error, isPending, isFetching, refetch } = useGitHubConfigQuery(
    ...
    { enabled: branchesReady && hasConnection }
  )
  ...
  return (
    <div className="flex h-full min-h-[560px] flex-col overflow-hidden rounded-lg border border-border bg-surface-100 text-foreground">
      ...
      {isPending || !branchesReady ? (
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center text-sm text-foreground-light">
          <p>Connect this project to a GitHub repository to read its config.toml.</p>
